...
if(isset($_POST['publish'])){
   ...
   $select_image = $conn->prepare("SELECT * FROM `" . TABLE_POSTS . "` WHERE image = ? ");
   ...
   if($select_image->rowCount() > 0 AND $image != ''){
      ...
      $insert_post = $conn->prepare("INSERT INTO `" . TABLE_POSTS . "`( titre, contenu,image,date) VALUES(?,?,?,?)");
      $insert_post->execute([ $titre, $content, $image,$date]);
