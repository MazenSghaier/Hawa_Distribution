...
if(isset($_POST['publish'])){
   ...
   $select_image = $conn->prepare("SELECT * FROM `".TABLE_REF_ACCUEIL."` WHERE image = ? ");
   ...
   if($select_image->rowCount() > 0 AND $image != ''){
      ...
      $insert_post = $conn->prepare("INSERT INTO `".TABLE_REF_ACCUEIL."`( titre, contenu,  image) VALUES(?,?,?)");
      $insert_post->execute([ $titre, $content,  $image]);
